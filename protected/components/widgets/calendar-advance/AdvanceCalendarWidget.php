...
class AdvanceCalendarWidget extends CWidget {
    ...
    public function drawCalendar() {
        ...
        for ($x = 0; $x < $running_day; $x++):
            $this->calendar.= '<td class="' . $this->style . '-day"> </td>';
            $days_in_this_week++;
        endfor;
        ...
        for ($list_day = 1; $list_day <= $days_in_month; $list_day++):
            ...
            foreach ($this->events as $key => $val) {
                $date = date("d-m-Y", $val[$this->udate_key]);
                $comp_date = $list_day . '-' . $month . '-' . $this->year;
                if ($date == $comp_date) {
//                    $this->calendar.= '<div class="' . $this->style . '-text my-key"><a href="">' . $val[$this->event_key] . '</a></div>';
                    $have_event = true;
                    $have_class = "have-event";
                }
            }

            if ($list_day == date("j", mktime(0, 0, 0, $this->month)) && date("n") == $this->month && date("Y") == $this->year) {
                $this->calendar.= '<td class="' . $this->style . '-current-day ' . $have_class . '">';
            } else {
                if (($running_day == "0") || ($running_day == "6")) {
                    $this->calendar.= '<td class="' . $this->style . '-day ' . $have_class . '">';
                } else {
                    $this->calendar.= '<td class="' . $this->style . '-day ' . $have_class . '">';
                }
            }
            ...
            if ($have_event == true) {
                $this->calendar.= '<div class="' . $this->style . '-day-number"><a href="javascript:void(0)" data-date="' . $this->year . '-' . $month . '-' . $list_day . '" class="correction_cal_view">' . $list_day . '</a></div>';
            } else {
                $this->calendar.= '<div class="' . $this->style . '-day-number"><a href="javascript:void(0)">' . $list_day . '</a></div>';
            }
