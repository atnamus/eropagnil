...
$(function() {
    tbody = $("tbody");
    tbody.val(getCalendar(year, month, day));
    wdaysOriginal();
...
function getFirstEvents() {
    $.ajax({dataType: "json", url: full_path+"/ajax-get-dates", type: "POST", data: {select: "%Y-%c-%e", where: "%Y-%c-%e", d: this_td.attr("id"), order: "DATE_FORMAT(timestamp, '%H-%i')"}, beforeSend: function() {
            $(".modal-body").html("<span class=\"loading\">Loading events...</span>")
        }, success: function(data) {
            getEvents(parseInt(this_td.children(".label").html()), data[this_td.attr("id")], this_td.attr("id"))
        }, error: function() {
            getEvents(null, null, this_td.attr("id"))
        }})
}
...
function getCalendar(y, m) {
    ...
    for (i = 1; i <= days_number; i++) {
        ...
        if (diap === days_number) {
            var numberOfWeeks = Math.ceil((days_number + empty) / 7), pre_total = days_number + empty, post_total = numberOfWeeks * 7;
    ...
    if ('sessionStorage'in window && window.sessionStorage !== null) {
        ...
        alert('What an old browser! The webpage won´t work correctly with this browser, update it!')
    }
    if (storage == sessionStorage && sessionStorage[y + "-" + m] !== undefined) {
        ...
        $.ajax({dataType: "json", url: "calendar/php/date.php", type: "POST", data: {select: "%Y-%c-%e", where: "%Y-%c", d: y + "-" + m, order: "timestamp, id"}, beforeSend: function() {
                $('#loading').show()
            }, success: function(data) {
    ...
    if (e !== null) {
        ...
        for (i = 0; i < showEvents.length; i++) {
            try {
                ...
                time = data[i].timestamp.replace("-", " ").replace("-", " ").replace(":", " ").replace(":", " ").split(" ");
                time = time.slice(3, 6);
                time = new Date(2000, 01, 01, time[0], time[1], time[2]);
                if (data[i].location === null) {
                    $(".modal-body").append("<div id='event'><span class='title'><h4 style=\"margin: 24px 0px\">" + data[i].title + "</h4></span><span class='time'><span style='width: 78px; float: left;'><span class='glyphicon glyphicon-time' style='top: 2px'></span> " + parseInt(time.getHours()) + ":" + (time.getMinutes() < 10 ? "0" + time.getMinutes() : time.getMinutes()) + (time.getHours() <= 12 ? "am" : "pm") + "</span></span></div>")
                } else {
                    $(".modal-body").append("<div id='event'><span class='title'><h4>" + data[i].title + "</h4><p>at <i>" + data[i].location + "</i></p></span><span class='time'><span style='width: 78px; float: left;'><span class='glyphicon glyphicon-time' style='top: 2px'></span> " + time.getHours() + ":" + (time.getMinutes() < 10 ? "0" + time.getMinutes() : time.getMinutes()) + (time.getHours() <= 12 ? "am" : "pm") + "</span></span></div>")
                }
            } catch (err) {
            }
        }
...
function addEvent(date) {
    ...
    date = new Date(date);
    date = (date.getMonth() + 1 < 10 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1) + "/" + (date.getDate() < 10 ? "0" + date.getDate() : date.getDate()) + "/" + date.getFullYear();
    $("#inputDate").val(date);
    $("button#add").on("click", function() {
        form = {title: $("#inputTitle").val(), location: $("#inputLocation").val(), date: $("#inputDate").val(), time: $("#inputTime").val()};
        title = form.title;
        loc = form.location;
        ...
        if ((title && loc && timestamp) !== '') {
            ...
            $.ajax({type: "POST", data: {action: "insert", title: form_data[1], loc: form_data[2], timest: form_data[3]}, url: "calendar/admin/php/func_events.php", beforeSend: function() {
                    ...
                    alert("There was an error trying to add the event.")
                }})
        } else {
